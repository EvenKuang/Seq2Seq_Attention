...
def trainIters(encoder, decoder, n_iters, reverse, print_every=1000, plot_every=100, learning_rate=0.01):
    ...
    # 修改pairs为train_pairs
    training_pairs = [tensorsFromPair(random.choice(train_pairs), reverse)
                      for i in range(n_iters)]
...
def evaluateOnTestSet(encoder, decoder, n=10):
    """在测试集上评估"""
    print(f"\n在测试集上评估({n}个样本）:")
    print("="*50)
    
    for i in range(min(n, len(test_pairs))):
        pair = test_pairs[i]  # 使用测试集
        print('>', pair[0])
        print('=', pair[1])
        output_words, attentions = evaluate(encoder, decoder, pair[0], reverse)
        output_sentence = ' '.join(output_words)
...
# 在测试集上测试
evaluateOnTestSet(encoder1, attn_decoder1)
...
if os.path.exists(font_path):
    ...
    print("中文字体文件未找到，使用默认字体")

def showAttention(input_sentence, output_words, attentions):
    # Set up figure with colorbar
    # plt.rcParams['font.sans-serif']=['SimHei'] # 会报错找不到，因为在系统目录中，函数上面部分的代码为添加路径
...
evaluateAndShowAttention("i am upset")

# 使用BLEU等机器翻译指标评估已训练好的模型
# from nltk.translate.bleu_score import sentence_bleu
# bleu_score = sentence_bleu([reference1, reference2, reference3], hypothesis1) # 数据集无多个reference
# print(bleu_score)
    ...
    if hypothesis and reference:
        try:
            ...
            if 0 <= score <= 1:
                ...
                # 显示前5个结果
                if i < 5:
                    print(f"{i+1}. 输入: {pair[0]}")
                    print(f"   参考: {pair[1]}")
                    print(f"   预测: {' '.join(hypothesis)}")
